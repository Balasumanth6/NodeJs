	...
	.then(favorites => {
		...
		for(const i of Object.keys(req.body)) {
			console.log(i, req.body[i]);
           	if(userFavorites.favDishes.find((d_id) => {
                if(d_id._id){
            ...
            userFavorites.favDishes.push(req.body._id);
        }
        userFavorites.save()
		.then((userFavorites) => {
			res.statusCode = 200;
...
.get(cors.corsWithOptions, authenticate.verifyUser, (req, res, next) => {
	...
	.then((favorites) => {
		if (favorites) {
			...
			console.log(favs);
            const dish = favs.favDishes.filter(favDish => favDish._id.toString() === req.params.dishId.toString())[0];
            console.log(dish);
            if (dish){
				res.statusCode = 200;
	            res.setHeader('Content-Type', 'application/json');
...
.post(cors.corsWithOptions, authenticate.verifyUser, (req, res, next) => {
	...
	.then((favorites) => {
		...
		if(!userFavorites) 
       	...
       	if(!userFavorites.favDishes.find((favDishId) => {
            if(favDishId._id){
                return favDishId._id.toString() === req.params.dishId.toString();
            }
        })) 
        	userFavorites.favDishes.push(req.params.dishId);
...
.delete(cors.corsWithOptions, authenticate.verifyUser, (req, res, next) => {
	...
	.then((favorites) => {
		...
		if (favorites)
			userFavorites = favorites.filter(fav => fav.user._id.toString() === req.user._id.toString())[0];
		if (userFavorites) {
			userFavorites.favDishes = userFavorites.favDishes.filter((dishid) => dishid._id.toString() !== req.params.dishId);
			userFavorites.save()
	        .then((result) => {
	            res.statusCode = 200;
	            res.setHeader("Content-Type", "application/json");
